...
class Phase7NewsTest < Minitest::Test
  ...
  def assert_image_matches(record, node, baseurl)
    images = node.css('.astra-news-image img')
    if record['image'] && !record['image'].empty?
      assert_equal 1, images.size
      assert_equal baseurl + record['image'], images.first['src']
      assert_equal record['image_alt'], images.first['alt']
    else
      assert_empty images
    end
  end
  ...
  def test_inventory_and_evidence_boundaries
    ...
    assert baseline.all? { |r| !r['image'] && !r['author'] && !r['published_at'] }
    records.select { |r| r['image'] && !r['image'].empty? }.each do |record|
      assert_match(%r{\A/assets/img/news/#{record['content_id']}/}, record['image'])
      refute_includes record['image'], '/cms/'
      refute_empty record['image_alt'].to_s.strip
    end
    assert_equal ['https://github.com/astra-vision/PaSCo', 'https://ieee-iv.org/2025/'], baseline.filter_map { |r| r['external_url'] }.sort
  ...
  def test_additional_published_news_item_accepts_a_public_cover_image_only
    Dir.mktmpdir('astra-news-cover-') do |root|
      item = records.find { |record| record['content_id'] == 'news-acvss-2025' }.merge(
        'content_id' => 'news-fixture-cover', 'slug' => 'fixture-cover', 'title' => 'Fixture cover News item',
        'image' => '/assets/img/news/news-fixture-cover/cover.gif', 'image_alt' => 'Fixture cover image')
  ...
  def test_precision_and_ranges
    ...
    image = '/assets/img/news/news-fixture/cover.gif'
    assert_raises(RuntimeError) { validate([original.merge('image' => image)]) }
    assert_raises(RuntimeError) { validate([original.merge('image' => 'cms/media/news/cover.gif', 'image_alt' => 'CMS path')]) }
  end

  def test_drafts_images_months_and_editorial_privacy_in_real_jekyll_fixture
    Dir.mktmpdir('astra-news-fixture-') do |root|
      %w[_news _projects _layouts _includes/news _data assets/img/news/fixture-image].each { |p| FileUtils.mkdir_p(File.join(root, p)) }
      ...
      FileUtils.cp(File.join(ROOT, 'assets/img/publication_preview/2023-pasco.gif'), File.join(root, 'assets/img/news/fixture-image/fixture.gif'))
      ...
      [base.merge('content_id' => 'fixture-image', 'slug' => 'fixture-image', 'image' => '/assets/img/news/fixture-image/fixture.gif', 'image_alt' => 'Fixture research preview', 'image_credit' => 'Fixture credit'),
      ...
      ['', '/astra-team.github.io'].each do |baseurl|
        ...
        assert_equal baseurl + '/assets/img/news/fixture-image/fixture.gif', with_image.at_css('img')['src']
  ...
  def test_generated_site
    ...
    archive.css('.astra-news-row').each do |row|
      assert_image_matches(published.find { |record| record['content_id'] == row['data-news-id'] }, row, baseurl)
    end
