(function () {
  ...
  function schedulePublicationStatusPolling(session, record, knownPull) { stopPublicationStatusPolling(); publicationStatusTimer = window.setTimeout(function () { loadPublicationStatus(session, record, knownPull); }, 15000); }
  async function publicationValidationState(session, pull) { var github = githubSettings(), base = '/repos/' + github.repoOwner + '/' + github.repoName; try { var checks = await githubResponse(base + '/commits/' + pull.head.sha + '/check-runs?filter=latest', session); var validation = (checks.check_runs || []).filter(function (check) { return check.name === 'CMS publication validation'; }); if (!validation.length || validation.some(function (check) { return check.status !== 'completed'; })) return 'pending'; return validation.every(function (check) { return ['success', 'neutral', 'skipped'].indexOf(check.conclusion) !== -1; }) ? 'passed' : 'failed'; } catch (_) { return 'unknown'; } }
  async function loadPublicationStatus(session, record, knownPull) { var epoch = ++publicationStatusEpoch, github = githubSettings(), base = '/repos/' + github.repoOwner + '/' + github.repoName, branch = publicationBranch(record.id), removal = unpublishBranch(record.id); stopPublicationStatusPolling(); try { var published = await publishedNewsByContentId(session, record.id), media = await draftMedia(session, record), pulls = await githubResponse(base + '/pulls?state=open&head=' + encodeURIComponent(github.repoOwner + ':' + branch), session), removals = await githubResponse(base + '/pulls?state=open&head=' + encodeURIComponent(github.repoOwner + ':' + removal), session); if (epoch !== publicationStatusEpoch) return; if (removals.length) { renderPublicationState('Unpublish submitted', 'Validation in progress…', removals[0].html_url); return; } if (pulls.length) { var pull = pulls[0], validation = await publicationValidationState(session, pull); if (epoch !== publicationStatusEpoch) return; var detail = validation === 'passed' ? 'Validation passed. Ready to publish.' : validation === 'failed' ? 'Validation failed.' : 'Validation in progress…'; if (!await publicationBranchMatchesDraft(session, record, branch, published, media)) detail = 'Draft changed after submission. The existing request remains unchanged. ' + detail; if (epoch !== publicationStatusEpoch) return; renderPublicationState(published ? 'Update submitted' : 'Submitted', detail, pull.html_url, publicNewsUrl(published)); if (validation === 'pending' || validation === 'unknown') schedulePublicationStatusPolling(session, record, pull); return; } if (published) { if (serializePublicNews(record, media, published) === published.markdown) renderPublicationState('Published', '', null, publicNewsUrl(published)); else renderPublicationState('Update available', '', null, publicNewsUrl(published)); return; } } catch (error) { if (epoch !== publicationStatusEpoch) return; if (knownPull) { renderPublicationState('Submitted', 'Validation in progress…', knownPull.html_url); schedulePublicationStatusPolling(session, record, knownPull); return; } renderPublicationState('Error/Conflict', friendlyError(error, 'Unable to determine publication status.')); return; } if (epoch === publicationStatusEpoch) renderPublicationState('Draft', ''); }
  ...
  async function submitGithubPublication() {
    ...
    try {
      ...
      if (pulls.length) { renderPublicationState(published ? 'Update submitted' : 'Submitted', 'Validation in progress…', pulls[0].html_url, publicNewsUrl(published)); await loadPublicationStatus(session, record, pulls[0]); return; }
      ...
      await loadPublicationStatus(session, record, pr);
  ...
  async function submitGithubUnpublish() {
    ...
    try {
      ...
      var pulls = await githubResponse(base + '/pulls?state=open&head=' + encodeURIComponent(github.repoOwner + ':' + branch), session);
      if (pulls.length) { renderPublicationState('Unpublish submitted', 'Validation in progress…', pulls[0].html_url); return; }
      await ensureLifecycleBranch(session, branch);
