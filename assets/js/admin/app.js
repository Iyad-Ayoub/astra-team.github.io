(function () {
  ...
  function schedulePublicationStatusPolling(session, record) { stopPublicationStatusPolling(); publicationStatusTimer = window.setTimeout(function () { loadPublicationStatus(session, record); }, 15000); }
  ...
  async function submitGithubPublication() {
    ...
    setPublicationBusy(true); publicationMessage('Submitting publication request…');
    try {
      ...
      if (pulls.length) { renderPublicationState(published ? 'Update submitted' : 'Submitted', 'Validation in progress…', pulls[0].html_url, publicNewsUrl(published)); await loadPublicationStatus(session, record); return; }
      ...
      await loadPublicationStatus(session, record);
